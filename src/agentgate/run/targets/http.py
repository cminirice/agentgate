...
class HttpTargetAdapter:
    ...
    def execute(self, request: TargetExecutionRequest) -> TargetExecutionResult:
        ...
        headers = _build_headers(request, credential)
        try:
...
def _build_body(request: TargetExecutionRequest) -> bytes:
...
def _build_headers(
    request: TargetExecutionRequest, credential: ResolvedCredential | None
) -> dict[str, str]:
    headers = {
    ...
    if request.turn_id is not None:
        headers["X-AgentGate-Turn-Id"] = request.turn_id
    if request.baggage:
