...
<template>
  <aside class="dataset-column dataset-list-panel">
    <div class="dataset-panel-heading">
      ...
      <ElButton type="primary" size="small" data-testid="create-dataset" @click="emit('create')">新建</ElButton>
    </div>
    <ElInput
      :model-value="query"
      clearable
    ...
    <div v-loading="loading" class="dataset-list">
      ...
      >
        ...
        <span class="dataset-badges">
          <ElTag v-if="item.purpose === 'regression'" size="small" type="danger" effect="plain">回归集</ElTag>
    ...
    <div class="dataset-list-actions">
      <ElButton size="small" @click="emit('import')">导入 JSON</ElButton>
      <ElDropdown v-if="items.find((item) => item.id === selectedId)" trigger="click">
        <ElButton size="small">更多</ElButton>
        <template #dropdown>
          <ElDropdownMenu>
            <ElDropdownItem @click="emit('copy', items.find((item) => item.id === selectedId)!)">复制测评集</ElDropdownItem>
            <ElDropdownItem divided @click="emit('archive', items.find((item) => item.id === selectedId)!)">归档测评集</ElDropdownItem>
          </ElDropdownMenu>
        </template>
      </ElDropdown>
    </div>
